		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					html += '			<div class="upload_choose hidden-xs hidden-sm">';
		            ...
		            html += '				<div id="status_info" class="info hidden-xs hidden-sm">选中0张文件，共0B。</div>';
		            html += '				<div class="btns">';
				    ...
				    html += '							<img class="upload_image" src="image/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						$("#uploadSuccessInf").append("<p>" + response + "</p>");
						$("#uploadSuccessInf").append("<input class='form-control' name='images' value='" + response + "' type='hidden'/>");
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
					...
					onFailure: function(file) {
						$("#uploadProgress_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
						$("#uploadFailureInf").append("<p>" + file.name + "</p>");
